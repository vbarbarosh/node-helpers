...
const redis_poll_zshift_callback_rpush = require('./redis_poll_zshift_callback_rpush');

describe('redis_poll_zshift_callback_rpush', function () {
    it('should skip valid-JSON non-object entries and keep polling', async function () {
        ...
        await assert.rejects(redis_poll_zshift_callback_rpush({
            ...make_options(),
            log,
            redis,
        ...
        assert.deepStrictEqual(processed, [valid]);
        ...
        assert.deepStrictEqual(redis.pushed[1].value, 'ok');
    });
});
